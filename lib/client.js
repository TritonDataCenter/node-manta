...
if (!MemoryStream)
    MemoryStream = require('readable-stream/passthrough.js');
var mime = require('mime');
var once = require('once');
var restify = require('restify');
var vasync = require('vasync');
var Watershed = require('watershed').Watershed;
var yakaa = require('yakaa');
    ...
    function onRequest(err, req) {
        if (err) {
            ...
            req.once('result', function () {
                /*
                 * Work around node ignoring keep-alive for HEAD:
                 */
...
function MantaClient(options) {
    ...
    if (options.proxy) {
        if (u.substring(0, 6) === 'https:') {
            options.agent = new yakaa.SSL({
                proxy: options.proxy,
                keepAlive: true
            });
        } else if (u.substring(0, 5) === 'http:') {
            options.agent = new yakaa({
                proxy: options.proxy,
                keepAlive: true
            });
        }
