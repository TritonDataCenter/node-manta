...
function getPath(p, skipEncode) {
    if (/^~~\//.test(p))
        p = p.replace(/^~~\//, '/' + process.env.MANTA_USER + '/');
...
MantaClient.prototype.job = function getJob(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.user) + (opts.path || '/live/status');
    var options = createOptions({
        accept: 'application/json',
        path: _path
    }, opts);
    ...
    signRequest({
        ...
        self.jsonClient.get(options, function (err2, _, __, obj) {
            if (err2) {
                if (err2.statusCode === 404 && !opts.path) {
                    opts.path = '/job.json';
                    self.job(j, opts, cb);
                    return;
                }
...
MantaClient.prototype.addJobKey = function addJobKey(j, k, opts, cb) {
    ...
    var keys = k.map(function (key) {
        ...
        if (/^\/.*\/stor\/.*/.test(key))
            return (key);
        return (getPath(key, true).replace(/\r?\n$/, ''));
    });
    var log = this.log.child({
        path: _path,
        req_id: options.id
...
MantaClient.prototype.jobInput = function getJobInput(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.user) + (opts.path || '/live/in');
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        marker: opts.marker || '',
...
MantaClient.prototype.jobOutput = function getJobOutput(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.user) + (opts.path || '/live/out');
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
...
MantaClient.prototype.jobFailures = function getJobFailures(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.user) + (opts.path || '/live/fail');
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
...
MantaClient.prototype.jobErrors = function getJobErrors(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.user) + (opts.path || '/live/err');
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
