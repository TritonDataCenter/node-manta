...
var PUBLIC_STOR_RE = /^\/(\w+)\/public/;
var REPORTS_STOR_RE = /^\/(\w+)\/reports/;
var STOR_RE = /^\/(\w+)\/stor/;
...
MantaClient.prototype.mkdirp = function mkdirp(dir, opts, cb) {
        ...
        if (dirs.length > 1 && dirs[1] == 'stor') {
                root = sprintf('/%s/stor', dirs[0]);
                dirs = dirs.slice(2);
        } else if (dirs.length > 3 && dirs[1] == 'jobs' && dirs[3] == 'stor') {
