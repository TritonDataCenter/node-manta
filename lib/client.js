...
function getPath(p, account, skipEncode) {
    if (/^~~\//.test(p))
        p = p.replace(/^~~\//, '/' + account + '/');
...
function getJobPath(p, account) {
    if (account && !/^\/.*\/jobs/.test(p))
        p = '/' + account + '/jobs/' + p;
...
function signRequest(opts, cb) {
    ...
    opts.sign('date: ' + opts.headers.date, function (err, obj) {
        ...
        var caller = obj.user ? obj.account + '/' + obj.user : obj.account;
 ...
 *  - account: optional account to create jobs under
 *  - user: optional user under the account
...
function MantaClient(options) {
    ...
    var caller = auth.parseCaller({
        account: options.account,
        user: options.user
    });

    this.account = caller.account;
    this.user = caller.user || caller.account;

    // Since this.user needs to remain backwards compatible, include this._user
    // for cases where you explicitly want the user. Add this._account for
    // consistency.
    this._user = caller.user;
    this._account = caller.account;
...
MantaClient.prototype.toString = function toString() {
    var user = this._user ? this._account + '/' + this._user : this._account;
...
MantaClient.prototype.chattr = function chattr(p, opts, cb) {
    ...
    var _path = getPath(p, this.account);
...
MantaClient.prototype.ftw = function ftw(p, opts, cb) {
    ...
    var _path = getPath(p, this.account, true);
    var options = createOptions({
        accept: '*/*',
        path: _path
    }, opts);
...
MantaClient.prototype.ln = function ln(src, p, opts, cb) {
    ...
    var _path = getPath(p, this.account);
    var _src = getPath(src, this.account);
    var options = createOptions({
        accept: 'application/json',
...
MantaClient.prototype.ls = function ls(dir, options, cb) {
    ...
    var directory = getPath(dir, this.account);
    ...
    var marker;
    var parent = getPath(dir, this.account, true);
    var returned = false;
    var self = this;
    ...
    if (!options._no_dir_check) {
        this.info(dir, function (info_err, meta, res) {
            if (info_err) {
                ...
                var err = new InvalidDirectoryError(directory);
                err.info = meta || {};
                err.info.parent = parent;
                cb(err, res);
            } else {
                next();
            }
...
MantaClient.prototype.mkdirp = function mkdirp(dir, opts, cb) {
    ...
    var d = getPath(dir, this.account, true);
    var dirs, root;
    var id = opts.req_id || libuuid.v4();
    var log = this.log.child({
        path: d,
...
MantaClient.prototype.createJob = function createJob(j, opts, cb) {
    ...
    assert.ok(caller.account || this.account, 'account must be specified');

    var _path = getJobPath('', caller.account || this.account);
    var options = createOptions({
        accept: 'application/json',
...
MantaClient.prototype.job = function getJob(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/status';
    var options = createOptions({
        accept: 'application/json',
        path: _path
    }, opts);
...
MantaClient.prototype.listJobs = function listJobs(opts, cb) {
    ...
    var _path = getJobPath('', this.account);
 ...
 * handed, like so:
 *
 * var client = manta.createClient({ ..., account: 'mark' });
 * var keys = [
 *   'foo',               // mark/stor/foo
 *   '/dave/stor/bar',
 * ];
...
MantaClient.prototype.addJobKey = function addJobKey(j, k, opts, cb) {
    ...
    var self = this;
    var _path = getJobPath(j, this.account) + '/live/in';
    var options = createOptions({
        accept: 'application/json',
    ...
    var keys = k.map(function (key) {
        ...
        if (/^\/.*\/stor\/.*/.test(key))
            return (key);
        return (getPath(key, this.account, true).replace(/\r?\n$/, ''));
    });
    var log = this.log.child({
        path: _path,
        req_id: options.id
...
MantaClient.prototype.cancelJob = function cancelJob(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/cancel';
    var options = createOptions({
        accept: 'application/json',
        contentLength: 0,
...
MantaClient.prototype.endJob = function endJob(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/in/end';
    var options = createOptions({
        accept: 'application/json',
        contentLength: 0,
        contentType: 'application/json',
...
MantaClient.prototype.jobOutput = function getJobOutput(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/out';
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
...
MantaClient.prototype.jobFailures = function getJobFailures(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/fail';
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
...
MantaClient.prototype.jobErrors = function getJobErrors(j, opts, cb) {
    ...
    var _path = getJobPath(j, this.account) + '/live/err';
    var emitter = new EventEmitter();
    var options = createOptions({
        accept: 'application/x-json-stream',
        path: _path
...
MantaClient.prototype.jobShare = function doJobShare(j, opts, cb) {
    ...
    var jobdir = getJobPath(j, this.account);
    jobshare({
        'client': this,
        'jobdir': jobdir,
...
MantaClient.prototype.signURL = function signURL(opts, cb) {
    ...
    var _opts = {
        ...
        path: getPath(opts.path, this.account, true),
        query: opts.query,
        role: opts.role,
        'role-tag': opts['role-tag'],
        sign: opts.sign || self.sign,
        account: opts.account || self.sign.account || self._account,
        user: opts.user || self.sign.user || self._user
    };
...
MantaClient.prototype.medusaAttach = function medusaAttach(j, opts, cb) {
        ...
        var _path = '/' + self.account + '/medusa/attach/' + j + '/master';
        var headers = {
                connection: 'upgrade',
                upgrade: 'websocket',
...
MantaClient.prototype.path = function _getPath() {
    return (getPath.apply(null, arguments));
};
