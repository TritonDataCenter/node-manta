...
function createOptions(opts, userOpts) {
    ...
    var options = {
        headers: utils.normalizeHeaders(userOpts.headers),
        id: id,
...
function getDefaultUserAgent() {
    ...
    return sprintf('node-manta/%s node/%s', pkg.version, process.versions.node);
}
...
function createRestifyClient(opts, type) {
    var client = restifyClients.createClient({
        ...
        type: type,
        userAgent: opts.userAgent || getDefaultUserAgent(),
        url: opts.url,
        version: '~1.0'
    });
...
function MantaClient(options) {
    ...
    this.client = createRestifyClient({
        ...
        headers: utils.normalizeHeaders(options.headers),
        log: self.log,
 ...
 *      - opts.skipEncode: (optional) a bool to pass through to `self.path()`
 *      - ...
...
MantaClient.prototype.get = function get(p, opts, cb) {
    ...
    assert.optionalBool(opts.skipEncode, 'opts.skipEncode');
    assert.func(cb, 'callback');

    var _path = this.path(p, opts.skipEncode);
    var bytes = 0;
    var etag;
...
MantaClient.prototype.put = function put(p, input, opts, cb) {
    ...
    var _path = this.path(p);
    var userHeaders = utils.normalizeHeaders(opts.headers);
...
MantaClient.prototype.rmr = function rmr(p, opts, cb) {
    ...
    var _opts = {
        headers: utils.normalizeHeaders(opts.headers),
        req_id: id,
