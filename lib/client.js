 ...
 */
function normaliseHeaders(hdrs) {
        if (!hdrs)
                return ({});
        var tmp = {};
        for (var hdr in hdrs) {
                if (hdrs.hasOwnProperty(hdr))
                        tmp[hdr.toLowerCase()] = hdrs[hdr];
        }
        return (tmp);
}
...
function createOptions(opts, userOpts) {
        ...
        var options = {
                headers: normaliseHeaders(userOpts.headers),
                id: id,
...
MantaClient.prototype.put = function put(p, input, opts, cb) {
        ...
        var _path = getPath(p);
        var userHeaders = normaliseHeaders(opts.headers);
...
MantaClient.prototype.rmr = function rmr(p, opts, cb) {
        ...
        var _opts = {
                headers: normaliseHeaders(opts.headers),
                req_id: id,
                limit: opts.limit || 1024,
                offset: 0
        };
