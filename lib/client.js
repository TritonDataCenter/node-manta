...
/* JSSTYLED */
var JOBS_STOR_RE = /^\/([a-zA-Z][a-zA-Z0-9_\.@]+)\/jobs\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\/stor/;
...
function getPath(p) {
        if (!STOR_RE.test(p) &&
            !PUBLIC_STOR_RE.test(p) &&
            !JOBS_STOR_RE.test(p) &&
            !REPORTS_STOR_RE.test(p)) {
...
MantaClient.prototype.mkdirp = function mkdirp(dir, opts, cb) {
        ...
        dirs.forEach(function (_d, i) {
                ...
                var _dir =
                        path.normalize(sprintf('/%s/%s/%s', root, tmp, _d));
...
MantaClient.prototype.rmr = function rmr(p, opts, cb) {
        ...
        var parallelism = opts.parallelism || 100;
        var outstanding = 0;
        ...
        function schedule(path, parent) {
                work.push({ path: path, parent: parent });
                reschedule();
        }

        function rmdir(p, parent) {
                var dir = { path: p, entries: 1, parent: parent, empty: true };

                self.ls(p, _opts, function (err, res) {
                        if (err) {
                                done(err);
                                return;
                        }
                        ...
                        res.on('object', function (ent) {
                                dir.entries++;
                                dir.empty = false;
                                schedule(p + '/' + ent.name, dir);
                        });

                        res.on('directory', function (ent) {
                                dir.entries++;
                                dir.empty = false;
                                rmdir(p + '/' + ent.name, dir);
                        });
                });
        }
...
MantaClient.prototype.cancelJob = function cancelJob(j, opts, cb) {
        ...
        var _path = getJobPath(j, this.user) + '/cancel';
        var options = createOptions({
                accept: 'application/json',
                contentLength: 0,
...
MantaClient.prototype.jobFailures = function getJobFailures(j, opts, cb) {
        ...
        var _path = getJobPath(j, this.user) + '/fail';
        var emitter = new EventEmitter();
        var options = createOptions({
                accept: 'application/x-json-stream',
                path: _path
...
MantaClient.prototype.jobErrors = function getJobErrors(j, opts, cb) {
        ...
        var _path = getJobPath(j, this.user) + '/err';
        var emitter = new EventEmitter();
        var options = createOptions({
                accept: 'application/x-json-stream',
                path: _path
