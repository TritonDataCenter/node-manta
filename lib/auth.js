...
function privateKeySigner(options) {
    ...
    assert.string(caller.account, 'options.account');
    ...
    sign.account = caller.account;
    sign.user = caller.user;
        ...
        sshAgentGetKey(agent, keyId, function (err, key) {
            ...
            sshAgentSign(agent, key, data, function (err2, res) {
                if (err2) {
                    ...
                    res.keyId = keyId;
                    res.account = caller.account;
                    res.user = caller.user;
                    cb(null, res);
                }
            });
        });
 ...
 *
 * Parameters:
 * - host, keyId, user, sign: see other client APIs
...
function signUrl(opts, cb) {
    ...
    assert.string(caller.account, 'options.account');
    assert.optionalString(caller.user, 'options.user');
    ...
    var login = caller.user ? caller.account + '/' + caller.user :
                              caller.account;
...
/**
 * Sorts out "account" and "user" taking backwards compatibility into
 * consideration.
 *
 * Pre-access control client used MANTA_USER == -a == --account == opts.account.
 * We want MANTA_ACCOUNT + MANTA_USER to make sense (i.e.,
 * MANTA_ACCOUNT == opts.account and MANTA_USER == opts.user) but doing so
 * breaks backwards compatibility because a user previously only set
 * MANTA_USER, and MANTA_ACCOUNT is required now. Instead, we still make
 * MANTA_ACCOUNT = opts.account and MANTA_USER = opts.user, but make sure to
 * handle everything appropriately by shuffling things around if needed.
 *
 * The following cases have been accounted for:
 * (1) MANTA_USER is set. MANTA_ACCOUNT is not set.
 *     This is the case where we want to maintain backwards compatibility.
 *     Assume MANTA_USER is the account login, unless it has a '/' character
 *     e.g. 'joyent/fred'. In that case, split it apart.
 * (2) MANTA_USER is not set. MANTA_ACCOUNT is set.
 *     The user wants to authenticate as an account owner.
 * (3) MANTA_USER is set. MANTA_ACCOUNT is set.
 *     The user wants to authenticate as a subuser. If MANTA_USER has the '/'
 *     character, drop the account part of MANTA_USER and use MANTA_ACCOUNT as
 *     the account.
 *
 * returns an object
 * {
 ...
 * }
 */
function parseCaller(opts) {
    assert.object(opts, 'options');
    assert.optionalString(opts.account, 'options.account');
    assert.optionalString(opts.user, 'options.user');

    var res = {};

    if (opts.user) {
        if (opts.user.indexOf('/') < 0) {
            if (opts.account) {
                res.user = opts.user;
            } else {
                res.account = opts.user;
            }
        } else {
            res.account = opts.user.substring(0, opts.user.indexOf('/'));
            res.user = opts.user.substring(opts.user.indexOf('/') + 1);
        }
    }

    if (opts.account) {
        // account option wins if account is specified in both opts.account and
        // opts.user with "account/user" style
        res.account = opts.account;
    }
