 ...
 * @param {Function} cb - `function (err, bodyStr)` where `err` is either
 *      (a) the given `httpErr` if no extra error info could be gleaned, or
 ...
 *      and where `bodyStr` is the error response body, if one was read.
 */
...
/*
 * Return an API path for the given bucket object **metadata** endpoint.
 *
 * @param {String} bucketName - Required. The bucket name.
 * @param {String} objectName - Required. The object name.
 */
MantaBucketsClient.prototype.bomPath = function bomPath(bucketName,
                                                        objectName) {
    assert.string(this.user, 'this.user');
    assert.string(bucketName, 'bucketName');
    assert.ok(bucketName.length, 'bucketName cannot be the empty string');
    ...
    return format('/%s/buckets/%s/objects/%s/metadata',
        this.user,
        encodeURIComponent(bucketName),
        encodeURIComponent(objectName));
};
            ...
            req.once('result', function onRes(resErr, res) {
                if (resErr) {
                    if (res && res.statusCode === 404) {
                        cb(new WError(resErr,
                            'bucket "%s" object "%s" does not exist',
 ...
 * (PUT /:login/buckets/:bucketName/objects/:objectName/metadata)
 *
 * Note: This is a full replacement of metadata for the object. If no
 * `opts.headers.m-*` values are given, then this will delete all custom
 * metadata for the given bucket object.
 *
 * @param {String} bucketName
 * @param {String} objectName
 * @param {Object} opts - Optional. Request options. See `mkBucketReqOpts`
 *      for defaults.
 *      - headers:
 *          - `m-*` headers are the object metadata to set.
 * @param {Function} cb - `function (err, res)`. If there is an error
 *      `err` will be set and the other args will be undefined. Otherwise,
 *      `res` is the response object with the headers.
 */
MantaBucketsClient.prototype.putBucketObjectMetadata =
function putBucketObjectMetadata(bucketName, objectName, opts, cb) {
    assert.string(bucketName, 'bucketName');
    assert.string(objectName, 'objectName');
    if (typeof (opts) === 'function') {
        cb = opts;
    ...
    var self = this;
    var reqOpts = mkBucketReqOpts(
        self.bomPath(bucketName, objectName),
        {
            log: self.log
        },
        opts);
    ...
    self.signRequest({
        ...
        self.client.put(reqOpts, function onReq(reqErr, req) {
            if (reqErr) {
                req.log.debug(reqErr, 'PutBucketObjectMetadata error');
                cb(reqErr);
                return;
            }

            // Node.js' http.request handling will add a Content-Length header
            // (with value 0) automatically, even if there is no data written.
            // This breaks the PutBucketObjectMetadata:
            ...
            req.once('result', function onRes(resErr, res) {
                ...
                res.once('end', function onEnd() {
                    switch (res.statusCode) {
                        ...
                        default:
                            cb(new VError(
                                'unexpected PutBucketObjectMetadata ' +
                                'response status: %s', res.statusCode));
                            break;
                    }
                });
...
module.exports = {
    ...
    readBucketApiErr: readBucketApiErr,
