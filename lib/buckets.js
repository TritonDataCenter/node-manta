...
function createBucket(bucketName, opts, cb) {
    ...
    var reqOpts = {
        ...
        path: self.path('~~/buckets/' + encodeURIComponent(bucketName))
    };
            ...
            req.once('result', function onRes(resErr, res) {
                if (resErr) {
                    if (res && res.statusCode === 404) {
                        ...
                        cb(new VError('unexpected CreateBucket error: %s', resErr));
                    }
                    return;
                }

                if (resErr) {
                    cb(new VError('unexpected DeleteBucket error: %s', resErr));
                    return;
                }
