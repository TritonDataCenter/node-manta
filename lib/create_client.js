...
var DEFAULT_OPTIONS = [
    ...
    },
    {
        ...
        type: 'string',
        env: 'MANTA_USER',
        help: 'Manta User ("account" or "account/user")',
        helpArg: 'USER'
    },
    {
        names: ['role'],
...
function checkBinEnv(opts) {
    ...
    if (!noAuth && !opts.account && !opts.user && !process.env.MANTA_USER &&
        !process.env.MANTA_ACCOUNT) {
...
/**
 * Replaces opts.account and opts.user with the results from parseCaller.
 * opts is modified when calling this function.
 */
function parseBinCaller(opts) {
    var caller = auth.parseCaller({
         account: opts.account,
         user: opts.user
    });
    opts.account = caller.account;
    opts.user = caller.user;
}
