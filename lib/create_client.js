...
function checkBinEnv(opts) {
    ...
    } else if (!process.env.MANTA_DEV_MODE) {
        var u = opts.url || process.env.MANTA_URL;
        if (MANTA_URLS.indexOf(u) === -1)
            throw new Error(u + ' is not a valid Manta URL');
    }
