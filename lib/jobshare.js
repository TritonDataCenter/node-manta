...
var urlpath = path.posix;  // manip of URL paths
var fs = require('fs');
...
function jStageFetchJob(j, cb)
{
    ...
    jobobj = urlpath.join(j.j_jobdir, 'job.json');
...
function jStageFetchJobDetails(j, cb)
{
    ...
    jFetchObjectList(j, barrier, urlpath.join(j.j_jobdir, 'in.txt'));
    jFetchObjectList(j, barrier, urlpath.join(j.j_jobdir, 'out.txt'));
    jFetchObject(j, barrier, urlpath.join(j.j_jobdir, 'err.txt'),
                function (data) {
...
function jFetchObjectList(j, barrier, objname)
{
    jFetchObject(j, barrier, objname, function (data) {
        data.split(/\n/).slice(0, j.j_maxobjects).forEach(function (l, i) {
            ...
            var base = urlpath.basename(objname);
