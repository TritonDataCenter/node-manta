...
function printObjectDefault(obj) {
    console.log(sprintf('%24s %10s %s',
        obj.mtime || '',
        obj.hasOwnProperty('size') ? obj.size : 'PRE',
        obj.name));
}
function printObjectHuman(obj) {
    console.log(sprintf('%24s %10s %s',
        obj.mtime || '',
        obj.hasOwnProperty('size') ? utils.prettyBytes(obj.size) : 'PRE',
        obj.name));
}
...
function do_ls(subcmd, opts, args, cb) {
    ...
    if (args.length === 0) {
        ...
        s = client.createListBucketsStream(streamOpts);
        s.on('readable', function onListBucketsReadable() {
            var bucket;
            while ((bucket = s.read()) !== null) {
                printBucket(bucket);
            }
        });
        s.once('error', function onListBucketsError(err) {
            cb(err);
        });
        s.once('end', function onListBucketsEnd() {
            cb();
        });
        ...
        // - the 'yadir' and 'yadir/...' case with both file and dir with
        //   same name
        ...
        //        [15:53:16 trentm@bluesteel:~/joy/node-manta2]
        //        $ aws s3 ls s3://trentm-play/yadir
        //                                   PRE yadir/
        ...
        //        [15:19:24 trentm@bluesteel:~/joy/electric-boray]
        ...
        //      - else, add a 'relname' field that one can used with
        //        '-o relname'
        ...
        s = client.createListBucketObjectsStream(muri.bucket, streamOpts);
        s.on('readable', function onListObjectsReadable() {
            var obj;
            while ((obj = s.read()) !== null) {
                printObject(obj);
            }
        });
        s.once('error', function onListObjectsError(err) {
            cb(err);
        });
        s.once('end', function onListObjectsEnd() {
            cb();
        });
    }
}
