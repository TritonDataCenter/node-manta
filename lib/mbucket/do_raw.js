...
function do_raw(subcmd, opts, args, cb) {
    ...
    // Note: We explicitly do not use MantaClient.path here,
    // `p = client.path(parsed.pathname)`. That function:
    //
    // 1. provides the "~~" home dir convenience,
    // 2. by default URL encodes each dir element, and
    // 3. breaks '/' unintentionally via its path normalization.
    //
    // #3 is definitely not wanted for a "raw" interface. #1 might be nice
    // but also argument isn't needed for a "raw" interface. #2 is arguably
    // a bug. FWIW, the MantaBucketsClient does not do this encoding.
    ...
    client.signRequest({
        ...
        client.client[methodFunc](reqOpts, function onRequest(reqErr, req) {
            ...
            if (!opts.data && (method === 'PUT' || method === 'POST')) {
                // Node.js http[s].request handling will typically add either
                // the `Content-Length` or `Transfer-Encoding: chunked` headers
                // for a PUT/POST request, **even if it has no data**. This
                // isn't what I understand `curl $url -X PUT` to do. This can
                // be problematic for an endpoint that doesn't accept a
                // Content-Length, e.g. PutBucketObjectMetadata.
                //
...
do_raw.help = [
    ...
    '',
    'Examples:',
    '    {{name}} {{cmd}} /$MANTA_USER/buckets/mahbukkit -X PUT   # CreateBucket',
    '    {{name}} {{cmd}} /$MANTA_USER/buckets        # ListBuckets',
    '    {{name}} {{cmd}} /$MANTA_USER/buckets -i     # ... with response headers',
    '    {{name}} {{cmd}} /$MANTA_USER/buckets -v     # ... with req and res headers',
    '    {{name}} -v {{cmd}} /$MANTA_USER/buckets 2> >(bunyan)  # TRACE bunyan logging',
    '    {{name}} {{cmd}} /$MANTA_USER/buckets?limit=2'
].join('\n');
