...
/*
 * Print info (headers) for a bucket object, from an API response object.
 *
 * Note: This intentionally does *not* include the "HTTP/..." start line
 * with the headers, because this "start-line" (per RFD 2616) isn't one
 * of the "headers". `minfo` *does* include the start line.
 *
 * The S3 CLI output for the "head-object" endpoint also encourages
 * excluding the start line info:
 *      $ aws s3api head-object --bucket trentm-play --key foo.txt
 *      {
 *          "AcceptRanges": "bytes",
 *          "ContentType": "text/plain",
 *          "LastModified": "Fri, 13 Sep 2019 19:10:02 GMT",
 *          "ContentLength": 3,
 ...
 *      }
 */
function printBucketObjectInfo(res) {
...
/*
 * Load and validate metadata from an array of args.
 *
 * See `metadataFromOpts` for documentation.
 */
function metadataFromArgs(args, log, cb) {
    ...
    vasync.forEachPipeline({
        inputs: args,
        func: function metadataFromArg(a, next) {
            log.trace({arg: a}, 'metadataFromArg');
            if (!a) {
                next(new cmdln.UsageError('empty metadata argument value'));
            } else if (a[0] === '{') {
                _addMetadataFromJsonStr(metadata, a, null, next);
            } else if (a[0] === '@') {
                _addMetadataFromFile(metadata, a.slice(1), next);
            } else {
                _addMetadataFromCommaSepKvStr(metadata, a, null, next);
            }
        }
    }, function (err) {
        if (err) {
...
module.exports = {
    ...
    printBucketObjectInfo: printBucketObjectInfo,
    metadataFromOpts: metadataFromOpts,
    metadataFromArgs: metadataFromArgs
};
