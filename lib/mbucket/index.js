...
// Tweaks to the "DEFAULT_CLI_OPTIONS" defined by the dir-style CLIs:
// - Remove RBAC-related options -- they aren't supported by the Buckets API.
// - Add some mbucket-specific (at least currently) options after "--verbose".
var options = manta.DEFAULT_CLI_OPTIONS.slice().filter(function (opt) {
    if (opt.env === 'MANTA_SUBUSER' || opt.env === 'MANTA_ROLE') {
        return false;
    } else {
        return true;
    }
...
function MBucketCli() {
    cmdln.Cmdln.call(this, {
        name: 'mbucket',
        desc: 'The Manta Buckets API command-line interface.',
        options: options,
        helpOpts: {
            includeEnv: true
        },
